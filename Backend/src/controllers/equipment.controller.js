...
class EquipmentController {
  ...
  createCategory = async (req, res) => {
    ...
    const duplicateExists = await categoryDuplicateExists({
      name,
      slug,
    });
  ...
  updateCategory = async (req, res) => {
    ...
    updateCategoryFields(category, req.body);
  ...
  createEquipment = async (req, res) => {
    ...
    const duplicateSlug = await equipmentSlugExists({
      slug,
    });
    ...
    const activeCategory = await findActiveCategory(category);
    ...
    try {
      uploadedImage = await uploadEquipmentImage({
        file: req.file,
        alt: imageAlt || title,
      });
...
getEquipmentStatistics = async (req, res) => {
  ...
  updateEquipment = async (req, res) => {
    ...
    const { slug, category, imageAlt } = req.body;

    if (slug !== undefined) {
      const duplicateSlug = await equipmentSlugExists({
        slug,

        excludedEquipmentId: equipment._id,
      });
    ...
    try {
      imageReplacement = await replaceEquipmentImage({
        equipment,
        file: req.file,
        imageAlt,

        fallbackAlt: req.body.title || equipment.title,
      });

      updateEquipmentFields(equipment, req.body);
      ...
      if (
        ...
        imageReplacement.oldPublicId !== imageReplacement.newPublicId
      ) {
        await deleteEquipmentImageSafely(imageReplacement.oldPublicId);
      }
      ...
      if (imageReplacement?.newPublicId) {
        await deleteEquipmentImageSafely(imageReplacement.newPublicId);
      }
