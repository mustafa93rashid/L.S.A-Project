...
const createFullBackup = async ({ onProgress } = {}) => {
  ...
  try {
    ...
    directories = await createTemporaryBackupDirectory();
    ...
    const mongoBackup = await createMongoBackup({
    ...
    console.log("[Backup] Starting Cloudinary backup...");

    const cloudinaryBackup = await createCloudinaryBackup({
      cloudinaryDirectory: directories.cloudinaryDirectory,
      assetsDirectory: directories.assetsDirectory,
      onProgress,
    });

    console.log(
      "[Backup] Cloudinary backup completed:",
      cloudinaryBackup.totalAssets,
      "assets",
    );
    ...
    console.log("[Backup] Creating backup info...");
    ...
    const fileName = `lsa-full-backup-${directories.timestamp}.zip`;
    ...
    const archiveResult = await createZipArchive({
      sourceDirectory: directories.rootDirectory,
    ...
    return {
      ...
      size: archiveResult.bytes,
      temporaryDirectory: directories.rootDirectory,
      ...
      cloudinary: {
        assetCount: cloudinaryBackup.totalAssets,
      },
    };
  } catch (error) {
    console.error("====================================");
    ...
    if (finalFilePath) {
      await removeFileSafely(finalFilePath);
    }

    if (directories?.rootDirectory) {
