...
import { Boxes, CheckCircle2, CircleOff, Filter, ImageIcon, Pencil, Plus, Search, ShieldCheck, ShieldX, Trash2, Truck, X } from 'lucide-react'
...
import { useDeleteEquipmentMutation, useEquipmentListQuery, useEquipmentStatisticsQuery } from '@/features/equipment/queries'
...
export default function EquipmentPage() {
  ...
  const statistics = statisticsData?.data.overview
        ...
        {isStatisticsLoading ? (
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {Array.from({ length: 4 }).map((_, index) => (
              <div key={index} className="h-[120px] animate-pulse rounded-[20px] border border-border/70 bg-muted/30" />
            ))}
          </div>
        ) : isStatisticsError ? (
          <div className="rounded-[22px] border border-border/70 bg-card px-6 py-8">
            <ErrorState description="Equipment statistics could not be loaded." onRetry={() => refetchStatistics()} />
          </div>
        ) : statistics ? (
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <StatCard index="01" label="Total Equipment" value={statistics.totalEquipment} icon={Truck} />
            <StatCard index="02" label="Available Units" value={statistics.totalAvailableUnits} icon={Boxes} />
            <StatCard index="03" label="Active Equipment" value={statistics.activeEquipment} icon={CheckCircle2} tone="success" />
            <StatCard index="04" label="Inactive Equipment" value={statistics.inactiveEquipment} icon={CircleOff} />
          </div>
        ) : null}
              ...
              {equipment.map((item) => {
                ...
                return (
                  <CollectionCard
                    ...
                    overlayLeft={item.category?.name ?? 'Uncategorized'}
                    overlayRight={`#${item.displayOrder}`}
                    eyebrow="Equipment"
                    icon={Truck}
                    title={item.title}
                    description={item.shortDescription || 'No equipment description available.'}
                    footerLeft={{
                      icon: Boxes,
                      label: 'Available',
                      value: `${item.availableUnits} ${item.availableUnits === 1 ? 'unit' : 'units'}`,
                    }}
                    footerRight={{
                      icon: item.safetyCertificate?.isAvailable ? ShieldCheck : ShieldX,
                      label: 'Certificate',
                      value: item.safetyCertificate?.isAvailable ? 'Certified' : 'Not available',
                      valueClassName: item.safetyCertificate?.isAvailable ? 'text-success' : undefined,
                    }}
