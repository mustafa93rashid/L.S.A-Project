...
import { Activity, CircleOff, Layers, Pencil, Plus, Trash2 } from 'lucide-react'
...
import { useDeleteEquipmentCategoryMutation, useEquipmentCategoriesQuery } from '@/features/equipment-categories/queries'
...
export default function EquipmentCategoriesPage() {
  ...
  const columns = useMemo<ColumnDef<EquipmentCategory, unknown>[]>(() => [
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-3">
          <div className="flex size-9 shrink-0 items-center justify-center rounded-xl border border-border/70 bg-muted/35 text-muted-foreground">
            <Layers className="size-4" strokeWidth={1.8} aria-hidden="true" />
          </div>
...

    {
      ...
      header: 'Slug',
      cell: ({ row }) => (
        <span className="inline-flex max-w-[220px] truncate rounded-md bg-muted/45 px-2 py-1 font-mono text-[11px] text-muted-foreground">
          {row.original.slug}
        </span>
      ),
    },
...

    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center justify-end gap-1">
          <Button type="button" variant="ghost" size="icon-sm" aria-label={`Edit ${row.original.name}`} className="text-muted-foreground hover:bg-muted hover:text-foreground" asChild>
            <Link to={`/equipment-categories/${row.original._id}/edit`}>
              <Pencil className="size-4" strokeWidth={1.8} />
            </Link>
          </Button>

          <Button type="button" variant="ghost" size="icon-sm" aria-label={`Delete ${row.original.name}`} className="text-muted-foreground hover:bg-destructive/10 hover:text-destructive" onClick={() => setDeletingCategory(row.original)}>
        ...
        {!isLoading && !isError ? (
          <section className="space-y-5">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <StatCard index="03" label="Inactive Categories" value={inactiveCategories} icon={CircleOff} />
            </div>
