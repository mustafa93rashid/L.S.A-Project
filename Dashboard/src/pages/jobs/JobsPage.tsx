...
import { Briefcase, CalendarDays, CheckCircle2, CircleOff, Clock3, Filter, MapPin, Pencil, Plus, Trash2, X } from 'lucide-react'
        ...
        <section>
          ...
          <div className="mb-5 rounded-[22px] border border-border/70 bg-card p-4 shadow-[0_1px_3px_rgba(0,0,0,0.025)] sm:p-5">
            <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
              ...
              <div className="relative min-w-0 flex-1">
                <Briefcase className="pointer-events-none absolute left-4 top-1/2 size-[18px] -translate-y-1/2 text-muted-foreground/45" strokeWidth={1.8} />
          ...
          {isLoading ? (
            ...
            <div className="rounded-[22px] border border-border/70 bg-card px-6 py-12">
              <ErrorState description="Job postings could not be loaded." onRetry={() => refetch()} />
            </div>
          ) : jobs.length === 0 ? (
            <div className="rounded-[22px] border border-border/70 bg-card px-6 py-12">
              <EmptyState icon={Briefcase} title="No job postings found" description="Try adjusting your filters, or add a new job posting." />
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {jobs.map((job) => (
                <CollectionCard
                  ...
                  image={
                    <div className="relative h-full w-full overflow-hidden bg-muted/30">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <Briefcase className="size-12 text-muted-foreground/[0.12]" strokeWidth={1.3} />
                      </div>
                    </div>
                  }

                  badges={
                    <Badge variant={job.status === 'published' ? 'success' : 'secondary'} className="shadow-sm">
                  ...
                  actions={
                    <>
                      <Button type="button" variant="ghost" size="icon-sm" aria-label={`Edit ${job.title}`} className="size-7 text-muted-foreground hover:bg-muted hover:text-foreground" asChild>
                      ...
                      {canDelete ? (
                        <Button type="button" variant="ghost" size="icon-sm" aria-label={`Delete ${job.title}`} className="size-7 text-muted-foreground hover:bg-destructive/10 hover:text-destructive" onClick={() => setDeletingJob(job)}>
